...
async def create_tenant(request: CreateTenantRequest, db: Session = Depends(get_db)):
    ...
    return Response(code="200", status="OK", message="Tenant created", result=tenant).dict(exclude_none=True)
...
async def get_tenant_by_id(tenant_id: str, db: Session = Depends(get_db)):
    ...
    return Response(code="200", status="OK", message="Tenant retrieved", result=tenant).dict(exclude_none=True) 

@tenant_router.get('/')
async def get_all_tenants(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    tenants = TenantService.get_all_tenants(db, skip, limit)
    return Response(code="200", status="OK", message="Tenants retrieved", result=tenants).dict(exclude_none=True)
...
async def update_tenant(tenant_id: str, request: UpdateTenantRequest, db: Session = Depends(get_db)):
    ...
    return Response(code="200", status="OK", message="Tenant updated").dict(exclude_none=True)
...
async def delete_tenant(tenant_id: str, db: Session = Depends(get_db)):
    ...
    return Response(code="200", status="OK", message="Tenant deleted").dict(exclude_none=True)
