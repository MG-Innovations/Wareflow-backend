...
class PaymentService:
    ...
    def get_all_payment_by_tenant_id(self, db: Session, tenant_id: UUID) -> List[Payment]:
        return db.query(Payment).filter(Payment.tenant_id == tenant_id).all()
    
    def get_all_payment_by_order_id(self, db: Session, order_id: UUID , tenant_id: UUID) -> List[Payment]:
        return db.query(Payment).filter(Payment.order_id == order_id , Payment.tenant_id == tenant_id).all()
